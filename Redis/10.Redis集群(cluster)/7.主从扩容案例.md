...
redis-cli -a 密码 --cluster $\textcolor{red}{\large add-node}$ 新增节点的ip:$\textcolor{red}{\large 6387}$ 任意一个已在集群的主节点的节点ip:$\textcolor{red}{\large 6381}$
...
重新分派槽号
命令:redis-cli -a 密码 --cluster reshard 任意已在集群的主节点ip:端口号
...
重新分配成本太高，所以前3家各自匀出来一部分，从6381/6383/6385三个旧节点分别匀出1367个坑位给新节点6387

![](images/38.槽号分配说明.png)

### 为新增主节点6387分配从节点6388

命令：redis-cli -a 密码  --cluster add-node 新slave的IP:新slave端口 新slave的IP:新master端口 --cluster-slave --cluster-master-id 新主机节点ID
